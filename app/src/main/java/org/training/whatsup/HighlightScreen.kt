...
fun HighlightScreen() {
    ...
    val eventListener = remember { mutableStateListOf<Event?>() }
    ...
    val onFirebaseQuerySuccess = { result: QuerySnapshot ->
        if (!result.isEmpty) {
            ...
            for (document in resultDocuments) {
                ...
                val startDate = event?.start_date?.toDate()?.let { dataFormatter.format(it) }
                val endDate = event?.end_date?.toDate()?.let { dataFormatter.format(it) }
                Toast.makeText(
                    screenContext,
                    "${event?.event_name} at ${event?.event_venue} ($startDate - $endDate)",
                    Toast.LENGTH_LONG
                ).show()
            } //end for
        } //end not empty
    ...
    AppTheme {
        Surface(
            ...
            Column(
                ...
                EventList(events = eventListener)
            } // end Column
        }
    }
}
    ...
    ElevatedCard(
        ...
        Column(modifier = Modifier.fillMaxWidth(1f).padding(8.dp)) {
            ...
            Row {
                ...
                Text(
                    text = dateFormatter.format(event.start_date?.toDate()),
                    style = TextStyle(
                        color = colorResource(id = R.color.md_theme_secondary),
                        fontSize = 18.sp
                    )
